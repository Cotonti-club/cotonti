...
if (empty($n)) {
    ...
    elseif ($a == 'add' && !empty($_POST)) {
        ...
        if (empty($rstructure['structure_code'])) {
            $rstructure['structure_code'] = preg_replace('#[^\w\p{L}\-]#u', '-', cot_translit_encode(mb_strtolower($rstructure['structure_title'])));
        }
        if ($rstructure['structure_path']) {
            $structure_count = $db->query("SELECT COUNT(*) FROM cot_structure where structure_path LIKE '{$rstructure['structure_path']}.%' AND structure_path NOT LIKE '{$rstructure['structure_path']}.%.%'")->fetchColumn();
            $rstructure['structure_path'] .= '.' . $structure_count + 1;
        } else {
            $rstructure['structure_path'] = 1;
        }
    ...
    elseif ($a == 'sort' && !empty($_POST["sort"])) {
        $items = json_decode($_POST["sort"], 1);
        if ($items)
            foreach ($items as $item) {
                $db->update($db_structure, ["structure_path" => $item["rpath"]], "structure_id = '{$item["id"]}'");
            }
        print json_encode([]);
        exit;
    }
    ...
    foreach ($sql->fetchAll() as $row) {
        ...
        $t->assign(array(
            ...
            'ADMIN_STRUCTURE_PATH' => cot_inputbox('hidden', 'rstructurepath[' . $structure_id . ']', $row['structure_path'], 'size="12" maxlength="255"'),
            'ADMIN_STRUCTURE_TPL_SYM' => $structure_tpl_sym,
            'ADMIN_STRUCTURE_TPLMODE' => cot_radiobox($check_tpl, 'rstructuretplmode[' . $structure_id . ']', array('1', '2', '3'), array($L['adm_tpl_empty'], $L['adm_tpl_parent'], $L['adm_tpl_forced']), '', ''),
    ...
    if (!$id && empty($al)) {
        ...
        $t->assign(array(
            ...
            'ADMIN_STRUCTURE_PATH' => cot_inputbox('text', 'rstructurepath', null, 'autocomplete="off" size="16" maxlength="16"'),
            'ADMIN_STRUCTURE_PARENT' => cot_selectbox_structure_tree($n, "", "rstructurepath", 1, "", 5),
