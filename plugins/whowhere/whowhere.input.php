...
if ($usr['id'] > 0 || (!$cfg['plugin']['whowhere']['disable_guests'] && !empty($usr['ip']))) {
    ...
    if (empty($params['r']) && (empty($params['e']) || $params['e'] == 'index')) {
        ...
        foreach ($whowhere_cfg as $code => $ww) {
            ...
            if (isset($ww['usl_empty']) && is_array($ww['usl_empty']))
                foreach ($ww['usl_empty'] as $k) {
                    if (!isset($params[$k])) $params[$k] = cot_import($k, 'G', 'TXT');
                    if (!empty($params[$k])) {
                        $usl = false;
                        break;
            ...
            if (isset($ww['usl_not_empty']) && is_array($ww['usl_not_empty']))
                foreach ($ww['usl_not_empty'] as $k) {
                    if (!isset($params[$k])) $params[$k] = cot_import($k, 'G', 'TXT');
                    if (empty($params[$k])) {
                        $usl = false;
                        break;
            ...
            if ($usl) {
                ...
                if (isset($ww['as']) && is_array($ww['as']))
                    foreach ($ww['as'] as $k => $v) {
