...
function cot_count_users()
{
    global $db, $db_users;
    return $db->query("SELECT COUNT(user_id) FROM $db_users WHERE user_maingrp NOT IN(5,6,3,2)")->fetchColumn();
}
function cot_count_inactive_users()
{
    global $db, $db_users;
    return $db->query("SELECT COUNT(user_id) FROM $db_users WHERE user_maingrp = ?", [2])->fetchColumn();
}