...
if ($a == 'update')
{
	...
	if ($rpagedelete)
	{
		...
		cot_redirect(cot_url('admin', "m=page&c=" . $row_page['page_cat'], '', true));
	}
	...
	{
		cot_redirect(cot_url('admin', "m=page&p=edit&id=$id", '', true));
	}
}
...
$mskin = cot_tplfile(array('page', 'admin', 'edit', $structure['page'][$pag['page_cat']]['tpl']));
...
$pageedit_array = array(
	...
	'PAGEEDIT_FORM_SEND' => cot_url('admin', "m=page&p=edit&a=update&id=".$pag['page_id']),
...
$catpatharray = cot_structure_buildpath('page', $pag['page_cat']);
$catpatharray[] = $L['page_edittitle'] . " #" . $pag['page_id'];
$catpath = cot_breadcrumbs($catpatharray, $cfg['homebreadcrumb'], true);

$t->assign("BREADCRUMBS", $catpath);
