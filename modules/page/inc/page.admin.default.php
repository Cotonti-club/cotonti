...
foreach ($sql_page->fetchAll() as $row) {
    ...
    $t->assign(array(
        ...
        'ADMIN_PAGE_EDIT_URL' => cot_url('admin', 'm=page&m=edit&id=' . $row['page_id']),
        'ADMIN_PAGE_URL_FOR_VALIDATED' => cot_url('admin', $common_params . '&a=validate&id=' . $row['page_id'] . '&d=' . $durl . '&' . cot_xg()),
        'ADMIN_PAGE_URL_FOR_UNVALIDATE' => cot_url('admin', $common_params . '&a=unvalidate&id=' . $row['page_id'] . '&d=' . $durl . '&' . cot_xg()),
        'ADMIN_PAGE_URL_FOR_DELETED' => cot_url('admin', $common_params . '&a=delete&id=' . $row['page_id'] . '&d=' . $durl . '&' . cot_xg()),
...
$t->assign(array(
    ...
    'ADMIN_PAGE_URL_ADD' => cot_url('admin', 'm=page&m=add'),
    ...
    'SEARCH_ORDER' => cot_selectbox($sorttype, 'sorttype', array_keys($sort_type), array_values($sort_type), false),
    'SEARCH_WAY' => cot_selectbox($sortway, 'sortway', array_keys($sort_way), array_values($sort_way), false),
    'SEARCH_FILTER' => cot_selectbox($filter, 'filter', array_keys($filter_type), array_values($filter_type), false),
    "SEARCH_CAT" => cot_page_selectcat($c, 'c'),
    "SEARCH_SQ" => cot_inputbox('text', 'sq', $sq),
