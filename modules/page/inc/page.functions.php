...
function cot_generate_pagetags($page_data, $tag_prefix = '', $textlength = 0, $admin_rights = null, $pagepath_home = false, $emptytitle = '')
{
    ...
    if ($page_data['page_id'] > 0 && !empty($page_data['page_title'])) {
        ...
        if (strlen($page_data['page_title']) >= 30)
            $page_link[] = $page_data['page_title'];
        else
            $page_link[] = $page_data['page_title'];
        ...
        $temp_array = array(
            ...
            'BREADCRUMBS' => $page_data['page_fulltitle'],
...
function cot_build_structure_page_tree($c)
{
    $tt = new XTemplate(cot_tplfile("page.tree.menu"));

    $items = cot_structure_tree("page");
    unset($items['system']);
